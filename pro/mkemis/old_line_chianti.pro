...
;	  -- GET_IONLIST
;	  -- SYMB2ZION [LAT2ARAB]
;	  -- RD_CHIANTI [READ_WGFA2(%), READ_ELVLC(%), READ_SPLUPS(%)]
;	  -- POPSOL [DESCALE_UPS(%)]
;	  -- TRANSLABEL
;	  -- KILROY
;	  -- IS_KEYWORD_SET
...
for i=0,nion-1 do begin				;for each ion...
  jon=ist[i]
  ...
  if nw gt 0 then begin
    ...
    for j=0,nt-1 do begin
      ...
      ;
      ;tmp=poplev(tt,pres,chline,n_e=n_e)
      tmp=popsol(tt,pres,chline,n_e=n_e, _extra=e)
      ;
      ...
      ;
      pop[j,0:npop-1]=popt[0:npop-1]*1e23
      			;(multiply by 1e23 just to make the numbers large)
      kilroy; was here.
    endfor	;j=0,nt-1
    print,''
    ...
    for j=0,ntrans-1 do begin
      wvl=ww[j] & l2=(chline.lev2)[j]-1 & a_val=(chline.a)[j]
	...
	if maxups gt 0. then upstst=1 else upstst=0
      ...
      if abs(wvl) gt wmn and abs(wvl) le wmx and upstst gt 0 then begin	;(
        ;	A -> cm -> hz -> erg -> keV
        if wvl ne 0 then nrg=((6.626d-27*(2.998d10/(wvl*1d-8)))/1.602d-9)
	...
	;------------------------------------------------------------;
        ...
        intens=dblarr(nt) & for k=0,nt-1 do intens[k]=hc*a_val*pop[k,l2]
	;
    ...
    if is_keyword_set(ow) then tmp=tmp[*,ow]
