function acisgarf_tmp,offset,wgrid,order=order,arm=arm,caldb=caldb,$
...
;		* if not given,
;			CALDBhrma/hrmaD1996-12-20effareaN0006.rdb
...
;		* if not given,
;			CALDBhetg/hetghegD1996-11-01greffpr001N0004.rdb
;	grfM	[INPUT] name of file containing MEG grating efficiencies
;		* if not given,
;			CALDBhetg/hetgmegD1996-11-01greffpr001N0004.rdb
;	grfL	[INPUT] name of file containing LEG grating efficiencies
;		* if not given,
;			CALDBletg/letgD1996-11-01greffpr001N0004.rdb
...
;	MAJOR CHANGES IN THE OFFING (VK; Jan04)
;-

message,'***	OBSOLETE: USE MKACISGARF!	***',/informational
...
if ok ne 'ok' then begin
  print,'Usage: effar=acisgarf_tmp(offset,wvlar,order=order,arm=arm,caldb=caldb,$'
...
if sz(nsz-2) eq 7 then hrmafil=strtrim(hrmaea(0),2) else $
	hrmafil=CALDBhrmacal+'cip/hrmaD1996-12-20effareaN0006.rdb'

;	QE data
sz=size(acissqe) & nsz=n_elements(sz) & nacisqe=n_elements(acissqe)
;if sz(nsz-2) eq 7 then acisqefil=acissqe else acisqefil=$
;	findfile('/soft/ciao/CALDB/data/chandra/acis/bcf/qe/acisD1997-04-17qeN0004.fits'
;	count=nacisqe)
acisqefil='/soft/ciao/CALDB/data/chandra/acis/bcf/qe/acisD1997-04-17qeN0004.fits'
nacisqe=1

;	grating efficiencies
sz=size(grfH) & nsz=n_elements(sz)
if sz(nsz-2) eq 7 then hegeff=strtrim(grfH(0),2) else $
	hegeff=CALDBhetgcal+'cip/hetghegD1996-11-01greffpr001N0004.rdb'
sz=size(grfM) & nsz=n_elements(sz)
if sz(nsz-2) eq 7 then megeff=strtrim(grfM(0),2) else $
	megeff=CALDBhetgcal+'cip/hetgmegD1996-11-01greffpr001N0004.rdb'
sz=size(grfL) & nsz=n_elements(sz)
if sz(nsz-2) eq 7 then legeff=strtrim(grfL(0),2) else $
	legeff=CALDBletgcal+'cip/letgD1996-11-01greffpr001N0004.rdb'
...
print,''
for i=0,5 do begin
  print,'Reading file '+acisqefil[0]+'['+strtrim(i+5,2)+']'
  tmp=mrdfits(acisqefil[0],i+5,hdr)
  nlin=n_elements(tmp.ENERGY) & var=dblarr(nlin)
  sNnrg[i,0:nlin-1L]=tmp.ENERGY
  sNqef[i,0:nlin-1L]=tmp.QE
endfor
