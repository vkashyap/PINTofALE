pro simeror, g, sims, bndu, bndl, clev=clev, alas=alas, _extra = e 
...
if ng eq 0 then ok='No g defined' else $
 if nmc lt 10 then ok='Please, more simulations.' 
if ok ne 'ok' then begin
  print,'Usage: simeror, g, sims, mcup, mcdown, clev=clev, /alas'  
if ok ne 'ok' then message,ok,/info
  return
endif
             
if keyword_set(clev) then begin
  if (clev gt 1.0) then begin 
